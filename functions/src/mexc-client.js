...
class MexcClient {
    constructor(apiKey, secret, sandbox = false) {
        ...
        this.rateLimiter = new shared_1.RateLimiter(15, 1000);
        this.circuitBreaker = new shared_1.CircuitBreaker(5, 60000);
    }
    /**
     ...
     */
    async fetchUniverse(filter, minVolume = 500000) {
     ...
     */
    async fetchKlines(symbol, timeframe, limit = 200, since) {
        return this.circuitBreaker.execute(async () => {
            await this.rateLimiter.waitIfNeeded();
     ...
     */
    async fetchMultipleKlines(symbols, timeframe, limit = 200) {
        const results = {};
