...
class ScoringEngine {
     ...
     */
    static calculateBurstScore(klines, indicators, universeVolatilities = []) {
     ...
     */
    static calculateVolatilityMetrics(klines, indicators, universeVolatilities = []) {
        const closes = klines.map(k => k.close);
        const volumes = klines.map(k => k.volume);
        // Realized volatility calculation
        const realizedVols = indicators_1.IndicatorCalculator.realizedVolatility(closes, 20);
        ...
        if (universeVolatilities.length > 0) {
            ...
            const universeMad = indicators_1.IndicatorCalculator.mad(universeVolatilities);
            volatilityZScore = indicators_1.IndicatorCalculator.zScore(realizedVolatility, universeMedian, universeMad);
        ...
        const rocValues = indicators_1.IndicatorCalculator.roc(closes, 3);
        ...
        const momentumNormalized = indicators_1.IndicatorCalculator.normalize(momentum);
        // Trend quality (ADX)
     ...
     */
    static calculateRawBurstScore(metrics) {
        const weights = env_1.defaultConfig.burstWeights;
        const volatilityComponent = indicators_1.IndicatorCalculator.normalize(metrics.volatilityZScore);
        return (weights.volatilityZScore * volatilityComponent +
     ...
     */
    static normalizeBurstScore(rawScore) {
        return Math.max(0, Math.min(100, 50 + 50 * Math.tanh(rawScore)));
    }
    /**
     * Determine signal state based on thresholds and conditions
     */
    static determineSignalState(klines, indicators, volatility, burstScore) {
        const thresholds = env_1.defaultConfig.stateThresholds;
        // Check ABOUT_TO_BURST conditions
        if (burstScore >= thresholds.aboutToBurst.burstScore &&
            ...
            const closes = klines.map(k => k.close);
            const recentVols = indicators_1.IndicatorCalculator.realizedVolatility(closes.slice(-10), 5);
        ...
        if (volatility.volatilityZScore >= thresholds.volatile.volatilityZScore ||
     ...
     */
    static calculateLeverageSuggestion(atrPercent) {
        const leverageMap = env_1.defaultConfig.leverageMap;
     ...
     */
    static detectStateTransition(previousState, currentState, symbol, timeframe, burstScore, volatilityScore) {
     ...
     */
    static calculateDeltas(current, previous) {
        if (!previous)
            return {};
        return {
     ...
     */
    static assessRisk(volatility, indicators, timeframe) {
        const riskFactors = [];
        let riskScore = 0;
        // Volatility risk
